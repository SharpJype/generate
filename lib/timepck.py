...
##def utcdelta(deltaobj, years=False, days=False, hours=False,
##             minutes=False, seconds=False, microseconds=False, string=False):
...
##        d[4] = 0
##    out = [(str(x)+s[i] if string else x) for i,x in enumerate(d) if b[i]]
##    if string: return "".join(out)
##    return out
...
##    if type(x)==str: # reverse
##        m = re.match(r'(?:(\d+)a)(?:(\d+)m)(?:(\d+)d)_(?:(\d+)h)(?:(\d+)m)(?:(\d+)s)', x)
##        if m:
##            return dtdt(tzinfo=tz.utc, year=int(found.group(1)),
...
PATHTIME_RE = r'(?:(\d+)\-)(?:(\d+)\-)(?:(\d+))_(?:(\d+)h)(?:(\d+)m)(?:(\d+)s)'
PATHTIME_RE_OLD = r'(?:(\d+)a)(?:(\d+)m)(?:(\d+)d)_(?:(\d+)h)(?:(\d+)m)(?:(\d+)s)'
def pathtime(date): # turn datetime object to a path friendly string or reverse it
    if type(date)==dtdt:
        return date.strftime(PATHTIME_STRFTIME)
    if type(date)==str and ((m:=re.match(PATHTIME_RE, date)) or (m:=re.match(PATHTIME_RE_OLD, date))): # reverse
